...
const areElementsSameCategory = (elements: Element[]): boolean => {
  ...
  const firstCategory = getElementCategory(elements[0]);
  return elements.every((element) => getElementCategory(element) === firstCategory);
};
...
export const useElementCategory = (elements: Element[]) => {
  return useMemo(() => {
    const mainCategory =
      elements.length === 1
        ? getElementCategory(elements[0])
        : areElementsSameCategory(elements)
          ? getElementCategory(elements[0]) // 多个元素同类型时，取第一个的类型作为代表
          : null; // 不同类型的返回空值
